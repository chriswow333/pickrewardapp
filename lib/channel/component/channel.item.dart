...
import 'package:pickrewardapp/shared/image/repo/viewmodel/image.dart';
...
class _ChannelLabelItemGroupState extends State<ChannelLabelItemGroup> {
  ...
  @override
  void initState(){
    widget.channelViewModel.initChannelModels("-1", 0);
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      key: channelViewModel.getShowItemGlobalKeys("-1"),
        ...
        child:Column(
          ...
          children:[
            ...
            GridView.builder(
              shrinkWrap:true,
              physics:const NeverScrollableScrollPhysics(),
              itemCount: channelViewModel.showLabelModels.length,
              gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                crossAxisCount: 4,
                mainAxisSpacing: 0,
                crossAxisSpacing: 10,
              ...
              itemBuilder: (context, index){
                return ChannelLabelItem(labelItemModel:channelViewModel.showLabelModels[index]);
              }
            ),
          ]
        ),
...
class ChannelLabelItem extends StatelessWidget {
  ...
  final ShowLabelModel labelItemModel;
...
class ChannelItemIcon extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if(ImageViewModel().hasImage("channel",channelItemModel.imageName)){
      return Stack(
      alignment:Alignment.center,
      children:[
        CircleAvatar(
          ...
          child:Image.memory(ImageViewModel().getImage("channel",channelItemModel.imageName))
        ),
    ...
    Future<Uint8List?> data = ImageViewModel().downloadImage("channel", channelItemModel.imageName);
    return Stack(
      alignment:Alignment.center,
      children:[
        CircleAvatar(
